...
async function onConversation() {
  ...
  try {
    ...
    const fetch0607Once = async () => {
      await fetch0607<Chat.ConversationResponse>({
        prompt: message,
        options,
        signal: controller.signal,
        onDownloadProgress: ({ event }) => {
          ...
          try {
            ...
            updateChat(
              ...
              dataSources.value.length - 1,
              {
                ...
                error: false,
                loading: false,
                conversationOptions: { conversationId: options.conversationId, parentMessageId: options.conversationId },
                requestOptions: { prompt: message, options: { ...options } },
              },
            )

            if (openLongReply && data.detail.choices[0].finish_reason === 'length') {
              options.parentMessageId = options.conversationId
              lastText = data.data
              message = ''
              return fetchChatAPIOnce()
            }
    ...
    await true ? fetch0607Once() : fetchChatAPIOnce()
  }
  catch (error: any) {
